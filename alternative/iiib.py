...
u = 1 # our choice of u
y = train[train['Standardised residuals'] > u]['Standardised residuals'] - u # construction of y
print(y)
...
fig, ax = plt.subplots()
ax.plot(xx, GPD_cdf(xxu, sol.x[0], sol.x[1]), 'k-', label='Fitted') # Plot the CDF of fitted GPD
ax.plot(y, empirical_cdf_obj(y), 'b*', label='Empirical') # Plot the empirical CDF of Y, the excess claim value above u
ax.legend()
ax.set_xscale('log')
ax.set_title('Empirical vs fitted CDF of the excess distribution')
